...
generic
(
  CLK_FREQ_MHZ   : integer   := 250; -- 250MHz GEN1, 500MHz GEN2
  TIMEOUT_US     : integer   := 5000;
  GUI_RUN        : integer   := 0;
  DEBUG_STOP     : integer   := 0;
  SERIALISER     : integer   := 0;
  PIPE           : integer   := 0;
  SCRAMBLING     : integer   := 1
);
...
constant DISABLE_8B10B      : integer   := PIPE - SERIALISER*PIPE;
constant DISABLE_SCRAMBLING : integer   := (1 - SCRAMBLING) * (1-SERIALISER);

constant IS_RC              : integer   := 0;
constant IS_EP              : integer   := 1;
constant RC_NODE            : integer   := 0;
constant EP_NODE            : integer   := 1;
...
signal   ClkOutRc           : std_logic;
signal   ClkOutEp           : std_logic;
signal   SerClkOutRc        : std_logic;
signal   SerClkOutEp        : std_logic;
...
-- ===============================================
-- SERIAL TEST INSTANTIATIONS
-- ===============================================
  g_GENSERIAL : if SERIALISER /= 0 generate

    -- Generate a serial clock
    P_CLKGENSER : process
    begin
      -- Generate a clock cycle
      loop
  ...
  -- -----------------------------------------------
  -- Instantiate PcieSwDispLinkSer for root complex
  -- -----------------------------------------------

  disprc : entity work.PcieSwDispLinkSer
  generic map
  (
  ...
  -- -----------------------------------------------
  -- Instantiate PcieSwDispLinkSer for Endpoint
  -- -----------------------------------------------

  dispep : entity work.PcieSwDispLinkSer
  generic map
  (
    LinkWidth                          => MAXLINKWIDTH,
    NodeNum                            => EP_NODE+10,
...
-- ===============================================
-- WIDE TEST INSTANTIATIONS
-- ===============================================
  else generate

  -- -----------------------------------------------
  -- Instantiate PcieSwDispLink for root complex
  -- -----------------------------------------------

  disprc : entity work.PcieSwDispLink
    generic map
    (
      LinkWidth         => MAXLINKWIDTH,
      NodeNum           => RC_NODE+10,
  ...
  dispep : entity work.PcieSwDispLink
    generic map
    (
      LinkWidth         => MAXLINKWIDTH,
      NodeNum           => EP_NODE+10,
